...

{
  ...
  "document_type": "original אם זה הדוח/ההודעה המקורית, או payment_demand אם זו דרישת תשלום שכבר כוללת תוספת פיגורים/ריבית (כלומר הדוח המקורי כבר עבר ויש חוב מצטבר)",
  ...
  "original_ticket_date": "תאריך ההודעה/המסמך הזה עצמו (YYYY-MM-DD) - התאריך שמופיע על הדוח שצולם. חשוב! ממנו נספרים 30 יום לערעור",
...
חשוב לגבי סוג המסמך (document_type):
- אם המסמך הוא הדוח המקורי או הודעה ראשונה על קנס - רשום "original"
- אם המסמך כולל תוספת פיגורים, ריבית, או סכום מעודכן שגבוה מהמקורי - רשום "payment_demand"
- סימנים ל-payment_demand: כתוב "דרישת תשלום", "הודעת חוב", יש עמודת "תוספת פיגורים", הסכום העדכני שונה מהמקורי

חשוב לגבי original_ticket_date:
- זה התאריך של המסמך שצולם - לא תאריך העבירה
- חפש תאריך הדפסה, תאריך הודעה, תאריך דרישה
- ממנו נספרים 30 יום לבקשה לביטול/ערעור
